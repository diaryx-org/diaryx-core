<script lang="ts">
  ...
  import { Settings, Info, Eye, Save, Check, AlertCircle, Loader2 } from "@lucide/svelte";
  import { getBackend } from "./backend";
  import { 
    getS3Config, storeS3Config, removeS3Credentials,
    getGoogleDriveRefreshToken, storeGoogleDriveRefreshToken, 
    getGoogleDriveFolderId, storeGoogleDriveFolderId,
    getGoogleDriveCredentials, storeGoogleDriveCredentials,
  ...
  let { open = $bindable(), showUnlinkedFiles = $bindable(), showHiddenFiles = $bindable(false), workspacePath = null }: Props = $props();
  ...
  let s3BackupProgress: { stage: string; percent: number } = $state({ stage: '', percent: 0 });
  ...
  let gdBackupProgress: { stage: string; percent: number } = $state({ stage: '', percent: 0 });
  ...
  async function loadSavedGoogleDriveConfig() {
    try {
      ...
      if ('getInvoke' in backend) {
        ...
        if (refreshToken) {
          try {
            const { refreshToken: refresh } = await import('@choochmeque/tauri-plugin-google-auth-api');
  ...
  async function signInWithGoogle() {
    ...
    try {
      ...
      if (!clientId) {
        alert('Google Client ID not found. Please ensure it is set in the backend environment.');
        return;
      }
      ...
      if (gdConfig.client_id || gdConfig.client_secret) {
        await storeGoogleDriveCredentials(gdConfig.client_id, gdConfig.client_secret);
      }
    ...
    try {
      ...
      if ('getInvoke' in backend) {
        ...
        try {
          ...
          unlisten = await listen<{ stage: string; percent: number; message: string | null }>('backup_progress', (event) => {
        ...
        const result = await invoke("backup_to_google_drive", {
          workspacePath: workspacePath ? workspacePath.substring(0, workspacePath.lastIndexOf('/')) : null,
          config: {
            name: gdConfig.name,
      ...
      gdBackupStatus = { success: false, message: e instanceof Error ? e.message : String(e) };
    } finally {
  ...
  async function testS3Connection() {
    ...
    try {
      ...
      if ('getInvoke' in backend) {
        const invoke = (backend as any).getInvoke();
        const result = await invoke("test_s3_connection", { config: {
          name: s3Config.name,
        ...
        s3TestResult = { success: result, message: result ? "Connection successful!" : "Connection failed" };
      } else {
        s3TestResult = { success: false, message: "S3 backup is only available in the desktop app" };
      }
    } catch (e) {
      s3TestResult = { success: false, message: e instanceof Error ? e.message : String(e) };
    } finally {
      s3Testing = false;
    }
  }
  ...
  async function backupToS3() {
    ...
    try {
      ...
      if ('getInvoke' in backend) {
        ...
        try {
          ...
          unlisten = await listen<{ stage: string; percent: number; message: string | null }>('backup_progress', (event) => {
            const { stage, percent, message } = event.payload;
            s3BackupProgress = { 
              stage: message || (stage === 'zipping' ? 'Creating zip archive...' : 
                                stage === 'uploading' ? 'Uploading to S3...' : 
                                stage === 'complete' ? 'Complete!' : 
                                stage === 'preparing' ? 'Preparing backup...' : stage),
              percent 
            };
          });
        } catch (e) { }
      ...
      s3BackupStatus = { success: false, message: e instanceof Error ? e.message : String(e) };
    } finally {
      if (unlisten) unlisten();
      s3IsBackingUp = false;
...
<Dialog.Root bind:open>
  <Dialog.Content class="sm:max-w-[500px] max-h-[80vh] overflow-y-auto">
    ...
    <div class="py-4 space-y-4">
        ...
        {:else}
          <div class="space-y-3 p-3 border rounded-lg bg-muted/30">
            {#if !selectedProvider}
              <!-- Provider Selection -->
              <div class="space-y-2">
                <Label for="backup-provider" class="text-sm font-medium">Select Backup Provider</Label>
                <select 
                  id="backup-provider"
                  class="w-full px-3 py-2 text-sm border rounded bg-background"
                  onchange={(e) => selectProvider((e.target as HTMLSelectElement).value as BackupProvider)}
                >
              ...
              <div class="space-y-3">
                ...
                <div class="grid grid-cols-2 gap-2">
                  <div class="space-y-1">
                    ...
                    <input id="s3-bucket" type="text" bind:value={s3Config.bucket} placeholder="my-backup-bucket" class="w-full px-2 py-1 text-sm border rounded bg-background" />
                  </div>
                  <div class="space-y-1">
                    <Label for="s3-region" class="text-xs">Region *</Label>
                    <input id="s3-region" type="text" bind:value={s3Config.region} placeholder="us-east-1" class="w-full px-2 py-1 text-sm border rounded bg-background" />
                  </div>
                </div>
                <div class="space-y-1">
                  <Label for="s3-prefix" class="text-xs">Prefix (optional)</Label>
                  <input id="s3-prefix" type="text" bind:value={s3Config.prefix} placeholder="backups/diaryx" class="w-full px-2 py-1 text-sm border rounded bg-background" />
                </div>
                <div class="space-y-1">
                  <Label for="s3-endpoint" class="text-xs">Custom Endpoint (for MinIO, etc.)</Label>
                  <input id="s3-endpoint" type="text" bind:value={s3Config.endpoint} placeholder="https://minio.example.com" class="w-full px-2 py-1 text-sm border rounded bg-background" />
                </div>
                <div class="grid grid-cols-2 gap-2">
                  <div class="space-y-1">
                    <Label for="s3-access-key" class="text-xs">Access Key *</Label>
                    <input id="s3-access-key" type="password" bind:value={s3Config.access_key} placeholder="AKIA..." class="w-full px-2 py-1 text-sm border rounded bg-background" />
                  </div>
                  <div class="space-y-1">
                    <Label for="s3-secret-key" class="text-xs">Secret Key *</Label>
                    <input id="s3-secret-key" type="password" bind:value={s3Config.secret_key} placeholder="••••••••" class="w-full px-2 py-1 text-sm border rounded bg-background" />
                  </div>
                </div>
                <div class="flex items-center gap-2 pt-2">
                  <Button variant="outline" size="sm" onclick={testS3Connection} disabled={s3Testing || !s3Config.bucket || !s3Config.access_key || !s3Config.secret_key}>{s3Testing ? 'Testing...' : 'Test Connection'}</Button>
                  <Button variant="default" size="sm" onclick={backupToS3} disabled={s3IsBackingUp || !s3Config.bucket || !s3Config.access_key || !s3Config.secret_key}>
                    {#if s3IsBackingUp}<Loader2 class="mr-2 size-4 animate-spin" />Backing up...{:else}Backup Now{/if}
                  </Button>
                </div>
                {#if s3IsBackingUp && s3BackupProgress.stage}
                  <div class="space-y-1">
                    <div class="flex items-center justify-between text-xs text-muted-foreground"><span>{s3BackupProgress.stage}</span><span>{s3BackupProgress.percent}%</span></div>
                    <Progress value={s3BackupProgress.percent} class="h-2" />
                  </div>
                {/if}
                {#if s3TestResult}<div class="flex items-center gap-2 text-sm">{#if s3TestResult.success}<Check class="size-4 text-green-500" /><span class="text-green-600">{s3TestResult.message}</span>{:else}<AlertCircle class="size-4 text-destructive" /><span class="text-destructive">{s3TestResult.message}</span>{/if}</div>{/if}
                {#if s3BackupStatus}<div class="flex items-center gap-2 text-sm">{#if s3BackupStatus.success}<Check class="size-4 text-green-500" /><span class="text-green-600">{s3BackupStatus.message}</span>{:else}<AlertCircle class="size-4 text-destructive" /><span class="text-destructive">{s3BackupStatus.message}</span>{/if}</div>{/if}
                {#if s3ConfigSaved}
                  <div class="flex items-center justify-between border-t pt-3 mt-3">
                    <span class="text-xs text-muted-foreground flex items-center gap-1"><Check class="size-3" />Credentials saved securely</span>
                    <Button variant="ghost" size="sm" onclick={clearSavedS3Config} class="text-xs text-muted-foreground hover:text-destructive">Clear saved</Button>
                  </div>
                {/if}
              </div>
              ...
              <div class="space-y-3">
                ...
                {#if !gdIsAuthenticated}
                  <div class="py-4 flex flex-col items-center gap-4">
                    ...
                    <Button variant="outline" onclick={signInWithGoogle} disabled={gdAuthLoading} class="w-full">
                      {#if gdAuthLoading}<Loader2 class="mr-2 size-4 animate-spin" />{:else}<span class="mr-2">G</span>{/if}
                      Sign in with Google
                    </Button>

                    <p class="text-[10px] text-muted-foreground text-center px-4">
                      Note: Your diary files will be stored in a dedicated folder in your Google Drive. 
                      Diaryx only accesses files it creates.
                    </p>
                  </div>
                {:else}
                  <div class="space-y-3">
                    <div class="flex items-center justify-between">
                      ...
                      <Button variant="ghost" size="sm" onclick={disconnectGoogleDrive} class="text-[10px] h-6 text-muted-foreground">Disconnect</Button>
                    </div>
                    ...
                    <div class="pt-2">
                       <Button variant="default" size="sm" onclick={backupToGoogleDrive} disabled={gdIsBackingUp} class="w-full">
                        {#if gdIsBackingUp}<Loader2 class="mr-2 size-4 animate-spin" />Backing up...{:else}Backup Now{/if}
                      </Button>
                    </div>
      ...
      {#if appPaths}
        <div class="space-y-2">
          <h3 class="font-medium flex items-center gap-2"><Info class="size-4" />App Paths</h3>
          <div class="bg-muted rounded p-3 text-xs font-mono space-y-1">
            {#each Object.entries(appPaths) as [key, value]}
              <div class="flex gap-2"><span class="text-muted-foreground min-w-[120px]">{key}:</span><span class="break-all">{value}</span></div>
            {/each}
          </div>
        </div>
      {/if}
      ...
      {#if config}
        <div class="space-y-2">
          <h3 class="font-medium flex items-center gap-2"><Settings class="size-4" />Config</h3>
          <div class="bg-muted rounded p-3 text-xs font-mono space-y-1">
            {#each Object.entries(config) as [key, value]}
              <div class="flex gap-2"><span class="text-muted-foreground min-w-[120px]">{key}:</span><span class="break-all">{typeof value === 'object' ? JSON.stringify(value) : String(value ?? 'null')}</span></div>
            {/each}
          </div>
        </div>
      {/if}
